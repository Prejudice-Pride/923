define([
    ...
    //SL_START///
        // TerrainEncoding.prototype.encode = function(vertexBuffer, bufferIndex, position, uv, height, normalToPack, webMercatorT) {
            TerrainEncoding.prototype.encode = function(vertexBuffer, bufferIndex, position, uv, height, normalToPack, webMercatorT,isHeightMap,fillTile) {
                //SL_END/////
        var u = uv.x;
        var v = uv.y;
        ...
        if (this.hasVertexNormals) {
              //SL_START/////
              if (isHeightMap)
              vertexBuffer[bufferIndex++] = normalToPack;//for slope when using heightmap
          else {
            if (fillTile){
                normalToPack.x = 123.456;
                normalToPack.y = 123.456;
            }
