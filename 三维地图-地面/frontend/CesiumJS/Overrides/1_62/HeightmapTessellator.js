define([
    ...
    //SL_START///
    var nScratchCartesian1 = new Cartesian3();
    var nScratchCartesian2 = new Cartesian3();
    var nScratchCartesian3 = new Cartesian3();
    //SL_END///
    ...
    HeightmapTessellator.computeVertices = function(options) {
        ...
        var uvs = new Array(size);
            // SL_START///////
        // We add normals when using heightMapTerrainData 
        ...
        //SL_START/////
        //Here we calculate each normal based on cross product of the neighboring vertices (if not underground mode)

        if (options.skirtHeight > 0){
            var vIndex = 0;
            var stride = width + 2;
            ...
            for (var row = 1; row < height+1; ++row) {
                ...
                for (var col = 1; col < width + 1 ; ++col) {
                    ...
                    vIndex = row * stride + col;
                    
                    var up = positions[(row+dy0)*stride + col];
                    var down = positions[(row+dy1)*stride + col];
                    var left = positions[row*stride + (col+dx0)];
                    var right = positions[row*stride + (col+dx1)];
    
                    var dU = Cartesian3.subtract(left, right, nScratchCartesian1);
                    var dV = Cartesian3.subtract(up, down, nScratchCartesian2);
                    var cross = Cartesian3.cross(dU, dV, nScratchCartesian3);
    
                    normals[vIndex] = AttributeCompression.octEncodeFloat(Cartesian3.normalize(cross, nScratchCartesian1));
                    
                }
            }
           
            //prevent skirting artefacts
           for(var ii=0;ii<35;ii++) {
             normals[0 + ii * stride] = normals[1 + ii * stride];
             normals[34 + ii * stride] = normals[33 + ii * stride];
             normals[ii + 0 * stride] = normals[ii + 1 * stride];
             normals[ii + 34 * stride] = normals[ii + 33 * stride];
           }
        }
        ...
        var aaBox = new AxisAlignedBoundingBox(minimum, maximum, relativeToCenter);
           //SL_START/////////
        //We tell Cesium that we would like to add the calculated normals for each vertex
        // var encoding = new TerrainEncoding(aaBox, hMin, maximumHeight, fromENU, false, includeWebMercatorT);
        var encoding = new TerrainEncoding(aaBox, hMin, maximumHeight, fromENU, (options.skirtHeight > 0 ? true : false ), includeWebMercatorT);
        //SL_END////////////
        ...
        for (var j = 0; j < size; ++j) {
               //SL_START///
            //We add the normals to the vertices buffer
            // bufferIndex = encoding.encode(vertices, bufferIndex, positions[j], uvs[j], heights[j], undefined, webMercatorTs[j]);
            bufferIndex = encoding.encode(vertices, bufferIndex, positions[j], uvs[j], heights[j], (options.skirtHeight > 0 ? normals[j] : undefined ), webMercatorTs[j],(options.skirtHeight > 0 ? true : undefined ));
            //SL_END////
        }
