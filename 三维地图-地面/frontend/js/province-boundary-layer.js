// 省界数据图层模块
// 使用Cesium官方GeoJsonDataSource加载显示省界线数据

class ProvinceBoundaryLayer {
  constructor(viewer) {
    this.viewer = viewer;
    this.dataSource = null;
    this.isVisible = false;
    this.apiUrl = '/api/data/province-boundary';

    // 省界样式配置
    this.boundaryStyle = {
      strokeWidth: 3, // 增加线宽
      strokeColor: Cesium.Color.LIME.withAlpha(0.8),
      clampToGround: false, // 禁用地面贴合以避免几何错误
      heightOffset: 35, // 离地面35米，介于国界和市界之间
      // 抗锯齿设置
      outline: true,
      outlineWidth: 1.0,
    ...
    // 显示高度控制
    this.displayHeights = {
      minHeight: 1000000,   // 1000km以上显示
      maxHeight: 10000000,  // 10000km以下显示
      optimalHeight: 5000000 // 最佳显示高度 5000km
    };
  }
  ...
  async initialize() {
    try {
      ...
      // 创建GeoJsonDataSource
      this.dataSource = new Cesium.GeoJsonDataSource('province-boundary');
      ...
      console.log('✅ 省界图层初始化成功');
      return true;
    } catch (error) {
      console.error('❌ 省界图层初始化失败:', error);
      return false;
    }
  }
  ...
  async loadData() {
    try {
      ...
      console.log(`📊 获取到 ${geojsonData.features.length} 个省界要素`);
      ...
      console.log('✅ 省界数据加载成功');
      return true;
    } catch (error) {
      console.error('❌ 省界数据加载失败:', error);
      return false;
    }
  }
  ...
  applyStyle() {
    ...
    console.log(`🎨 已应用省界样式到 ${entities.length} 个要素`);
  }
  ...
  createInfoWindow() {
    ...
    this.infoWindow.className = 'cesium-province-info-window';
  ...
  handleMouseClick(event) {
    ...
    if (pick && pick.id && pick.id.polyline && this.isProvinceBoundaryEntity(pick.id)) {
   ...
   * 检查是否为省界实体
   */
  isProvinceBoundaryEntity(entity) {
  ...
  generateInfoWindowContent(properties) {
    if (!properties || !properties.getValue) {
      return '<div><strong>🗾 省界线</strong><br/>无详细信息</div>';
    }

    const now = Cesium.JulianDate.now();

    // 获取主要属性
    const name = this.getPropertyValue(properties, ['NAME', 'name', '名称', 'Province', 'PROV_NAME'], now) || '未知省份';
    const code = this.getPropertyValue(properties, ['CODE', 'code', '代码', 'PROV_CODE'], now);
    const area = this.getPropertyValue(properties, ['AREA', 'area', '面积'], now);
    const capital = this.getPropertyValue(properties, ['CAPITAL', 'capital', '省会'], now);

    // 构建HTML内容
    let content = `<div><strong>🗾 省界线</strong><br/>`;
    content += `<strong>省份:</strong> ${name}<br/>`;

    if (capital && capital !== '未知') {
      content += `<strong>省会:</strong> ${capital}<br/>`;
    }
    ...
    content += `<strong>级别:</strong> 省级边界<br/>`;
    content += `</div>`;
  ...
  setVisible(visible) {
    ...
    console.log(`👁️ 省界图层${visible ? '显示' : '隐藏'}`);
  }
  ...
  zoomToLayer() {
    ...
    try {
      ...
      console.log('🎯 已缩放到省界图层范围');
    } catch (error) {
      console.error('❌ 缩放到省界图层失败:', error);
    }
  }
  ...
  getLayerInfo() {
    ...
    return {
      ...
      dataSource: 'API: /api/province-boundary',
  ...
  destroy() {
    ...
    this.isVisible = false;
    console.log('🗑️ 省界图层已销毁');
  }
}

// 导出到全局
window.ProvinceBoundaryLayer = ProvinceBoundaryLayer;